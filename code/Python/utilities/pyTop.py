...
DEFAULT_CONF: Template = Template(f'#? Config file for pyTop v. {VERSION}' + '''

#* Color theme, looks for a .theme file in "/usr/[local/]share/pyTop/themes" and "~/.config/pyTop/themes", "Default" for builtin default theme.
...
#* Set to 2 or higher to greatly decrease pyTop cpu usage. (Only integers)
...
#* Enable check for new version from github.com/aristocratos/pyTop at start.
update_check=$update_check

#* Set loglevel for "~/.config/pyTop/error.log" levels are: "ERROR" "WARNING" "INFO" "DEBUG".
#* The level set includes all lower levels, i.e. "DEBUG" will show all logging info.
log_level=$log_level
''')

CONFIG_DIR: str = f'{os.path.expanduser("~")}/.config/pyTop'
if not os.path.isdir(CONFIG_DIR):
	try:
		...
		raise SystemExit(1)
CONFIG_FILE: str = f'{CONFIG_DIR}/pyTop.conf'
THEME_DIR: str = ""

if os.path.isdir(f'{os.path.dirname(__file__)}/pyTop-themes'):
	THEME_DIR = f'{os.path.dirname(__file__)}/pyTop-themes'
elif os.path.isdir(f'{os.path.dirname(__file__)}/themes'):
	THEME_DIR = f'{os.path.dirname(__file__)}/themes'
else:
	for td in ["/usr/local/", "/usr/", "/snap/pyTop/current/usr/"]:
		if os.path.isdir(f'{td}share/pyTop/themes'):
			THEME_DIR = f'{td}share/pyTop/themes'
			break
...
class Config:
	...
	def __init__(self, path: str):
		...
		if not "version" in conf.keys():
			...
			self.recreate = True
			self.info.append(f'Config file version and pyTop version mismatch, will be recreated on exit!')
	...
	def load_config(self) -> Dict[str, Union[str, int, bool]]:
		...
		elif SYSTEM == "BSD" and os.path.isfile("/usr/local/etc/pyTop.conf"):
			conf_file = "/usr/local/etc/pyTop.conf"
		elif SYSTEM != "BSD" and os.path.isfile("/etc/pyTop.conf"):
			conf_file = "/etc/pyTop.conf"
		else:
			return new_config
		try:
...
try:
	...
	errlog.info(f'New instance of pyTop version {VERSION} started with pid {os.getpid()}')
...
class Banner:
	'''Holds the pyTop banner, .draw(line, [col=0], [center=False], [now=False])'''
	out: List[str] = []
	c_color: str = ""
	length: int = 0
	if not out:
...
class Menu:
		...
		while not cls.close:
			...
			if cls.resized:
				...
				if UpdateChecker.version != VERSION:
					banner += f'{Mv.to(Term.height, 1)}{Fx.b}{THEME.title}New release {UpdateChecker.version} available at https://github.com/aristocratos/pyTop{Fx.ub}{Term.fg}'
				cy = 0
	...
	def help(cls):
		...
		help_items: Dict[str, str] = {
			...
			"_3" : "https://github.com/aristocratos/pyTop",
		}
		...
		categories: Dict[str, Dict[str, List[str]]] = {
			"system" : {
				"color_theme" : [
					...
					'"/usr/[local/]share/pyTop/themes" and',
					'"~/.config/pyTop/themes".',
					'',
					...
					'https://github.com/aristocratos/pyTop'],
				"theme_background" : [
			...
			"proc" : {
				"proc_update_mult" : [
					...
					'',
					'Set to 2 or higher to greatly decrease pyTop',
...
class UpdateChecker:
	...
	def _checker(cls):
		try:
			with urllib.request.urlopen("https://github.com/aristocratos/pyTop/raw/master/pyTop.py", timeout=5) as source: # type: ignore
				for line in source:
			...
			if cls.version != VERSION and which("notify-send"):
				try:
					subprocess.run(["notify-send", "-u", "normal", "pyTop Update!",
						f'New version of pyTop available!\nCurrent version: {VERSION}\nNew version: {cls.version}\nDownload at github.com/aristocratos/pyTop',
...
def now_awake(signum, frame):
	...
	Draw.now(Term.alt_screen, Term.clear, Term.hide_cursor, Term.mouse_on, Term.title("pyTop"))
	Term.echo(False)
	Key.start()
	Term.refresh()
	Box.calc_sizes()
...
def clean_quit(errcode: int = 0, errmsg: str = "", thread: bool = False):
	...
	if errcode == 0:
		...
		if not errmsg: errmsg = f'pyTop exited with errorcode ({errcode}). See {CONFIG_DIR}/error.log for more information!'
