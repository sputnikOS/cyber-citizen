...
# https://github.com/sputnikOS/open-source
print_info() {
    info title
    info underline
...
# fetch --block_range 0 7
#
# Display colors 0-15 in the blocks. (16 colors)
# fetch --block_range 0 15
block_range=(0 15)
...
# text.
# Example:
# col_offset="auto" - Default behavior of fetch
...
#
# Example:
# fetch --bar_colors 3 4
# fetch --bar_colors distro 5
...
#
# Default: 'barinfo --memory_display'
...
#           'command output (fetch --ascii "$(fortune | cowsay -W 30)")'
# Flag:     --source
#
...
# Image loop
# Setting this to on will make fetch redraw the image constantly until
...
# Thumbnail directory
#
# Default: '~/.cache/thumbnails/fetch'
# Values:  'dir'
thumbnail_dir="${XDG_CACHE_HOME:-${HOME}/.cache}/thumbnails/fetch"
...
# https://github.com/dylanaraps/fetch/wiki/What-is-Waifu-Crop%3F
crop_mode="normal"
...
get_gpu() {
    case $os in
        ...
        "Mac OS X"|"macOS")
            if [[ -f "${cache_dir}/fetch/gpu" ]]; then
                source "${cache_dir}/fetch/gpu"
...
make_thumbnail() {
    ...
    mkdir -p "${thumbnail_dir:=${XDG_CACHE_HOME:-${HOME}/.cache}/thumbnails/fetch}"
...
get_user_config() {
    ...
    elif [[ -f "${XDG_CONFIG_HOME}/fetch/config.conf" ]]; then
        source "${XDG_CONFIG_HOME}/fetch/config.conf"
        err "Config: Sourced user config.    (${XDG_CONFIG_HOME}/fetch/config.conf)"

    elif [[ -f "${XDG_CONFIG_HOME}/fetch/config" ]]; then
        source "${XDG_CONFIG_HOME}/fetch/config"
        err "Config: Sourced user config.    (${XDG_CONFIG_HOME}/fetch/config)"

    elif [[ -z "$no_config" ]]; then
        config_file="${XDG_CONFIG_HOME}/fetch/config.conf"

        # The config file doesn't exist, create it.
        mkdir -p "${XDG_CONFIG_HOME}/fetch/"
        printf '%s\n' "$config" > "$config_file"
    fi
}
...
cache() {
    if [[ "$2" ]]; then
        mkdir -p "${cache_dir}/fetch"
        printf "%s" "${1/*-}=\"$2\"" > "${cache_dir}/fetch/${1/*-}"
    fi
}
...
cache_uname() {
    ...
    if [[ "$kernel_name" == "Darwin" ]]; then
        ...
        # https://github.com/dylanaraps/fetch/issues/1607
...
usage() { printf "%s" "\
Usage: fetch func_name --option \"value\" --option \"value\"

fetch is a CLI system information tool written in BASH. fetch
...
INFO:
    func_name                   Specify a function name (second part of info() from config) to
                                ...
                                Example: fetch uptime --uptime_shorthand tiny

                                Example: fetch uptime disk wm memory

                                This can be used in bars and scripts like so:

                                memory=\"\$(fetch memory)\"; memory=\"\${memory##*: }\"

                                For multiple outputs at once (each line of info in an array):

                                IFS=\$'\\n' read -d \"\" -ra info < <(fetch memory uptime wm)
    ...
    --disable infoname          Allows you to disable an info line from appearing
                                ...
                                NOTE: You can supply multiple args. eg. 'fetch --disable cpu gpu'
...
IMAGE BACKEND:
    ...
    --ascii source              Shortcut to use 'ascii' backend.
                                ...
                                NEW: fetch --ascii \"\$(fortune | cowsay -W 30)\"
...
DEVELOPER:
    --gen-man                   Generate a manpage for fetch in your PWD. (Requires GNU help2man)


Report bugs to https://github.com/dylanaraps/fetch/issues
...
main() {
    ...
    # Add fetch info to verbose output.
    err "fetch command: $0 $*"
    err "fetch version: $version"
